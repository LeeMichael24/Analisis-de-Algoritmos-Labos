...
/* void convert_string_to_char(string str, char* c){
    for(int i = 0; i < str.length(); ++i)
        c[i] = str[i];
}

void asignation(Battalion* b, string code, int id, int manpower, int armoredEquipment, double budget){
    b->setCode(code);
    b->setId(id);
    b->setManpower(manpower);
    b->setArmoredEquipment(armoredEquipment);
    b->setBudget(budget);
} */
